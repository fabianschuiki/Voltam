...
{
	Path * p = new Path;
	p->nodes.push_back((Path::Node){ 0,  0, true});
	p->nodes.push_back((Path::Node){50, 50, false});
	p->nodes.push_back((Path::Node){0, 50, false});
...
bool EditorWidget::on_draw(const Cairo::RefPtr<Cairo::Context>& cr)
{
	...
	for (GeometrySet::iterator g = geometries.begin(); g != geometries.end(); g++) {
		Layer * l = (*g)->layers[0];
		if (!l) continue;
